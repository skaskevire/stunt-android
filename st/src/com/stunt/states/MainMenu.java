...
public class MainMenu extends GameState
{
	...
	{
		for (Entry<Integer, MyInput.MonitorCoordinate> entry : MyInput.getActivePointers()
				.entrySet())
		{
			if (entry.getValue().getX() > Globals.V_WIDTH / 2.5f - Globals.V_WIDTH / 20
					&& entry.getValue().getX() < Globals.V_WIDTH / 2.5f + Globals.V_WIDTH / 20
					&& entry.getValue().getY() < Globals.V_HEIGHT / 1.8f
					&& entry.getValue().getY() > Globals.V_HEIGHT / 2.5f - Globals.V_HEIGHT / 28)
			{
		...
		if (MyInput.getActivePointers().values().size() == 0)
		{
			...
			if (playTapped == true)
			{
				gsm.setState(Globals.CHOOSE_LEVEL_MENU_GS, null, null);
			}
		}
